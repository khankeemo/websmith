...
| **Admin Messages Query Inbox — Two-Way Conversation + Inbound Email + Public Email Branding Cleanup (AWS-01 R01, 2026-08-15)** | ✅ Applied (builds on the R01 Query Inbox redesign: a REAL two-way client↔admin conversation. **Canonical `messages[]` thread** on the same ticket document (no new collection/system): `{id, senderType, direction, senderEmail, senderName?, recipientEmail, message, createdAt, source, deliveryStatus?, deliveryError?, providerMessageId?, inReplyTo?, references?}` — seeded by Get in Touch (`app/api/tickets/public/route.ts` initial client message + `lastClientReplyAt` + `adminReadAt:null`), appended by admin replies (`[id]/replies/route.ts` — outbound entry with Brevo `providerMessageId` + honest `deliveryStatus: sent|failed|not_sent`, `adminReadAt`), and by `send-resolution-email` / `send-client-portal-access` / `resend` routes (outbound entries + `adminReadAt`); `history` stays the audit log (Resend snapshots, status changes). **NEW `POST /api/tickets/inbound`** (admin-auth): reads enabled PG `mailboxes` READ-ONLY via `getDb` (`SELECT … WHERE is_enabled = TRUE` — no mailbox row/credential/SMTP/schema ever changed), IMAP-fetches UNSEEN (mailbox opened READ-ONLY, never marks Seen so the Internal Communications Center sync of the same mailbox is unaffected), parses with `mailparser`, matches tickets by **In-Reply-To/References against stored `messages.providerMessageId`** (fallback: sender email == ticket `contactEmail`, NEVER subject alone), ALWAYS verifies the sender, dedupes by the inbound Message-ID, stores the client message + `client_reply` history + `lastClientReplyAt`, reopens `closed → in_progress`; returns an honest per-mailbox summary and a clear `NO_MAILBOXES` message when none are configured. **Unread**: `GET /api/tickets` now computes `hasNewClientReply` (`lastClientReplyAt > adminReadAt`, server-derived) and NEW `POST /api/tickets/[id]/read` stamps `adminReadAt` (UI auto-marks read on open + unread dot on rows). **UI** (`AdminMessagesClient.tsx`): client/admin thread **bubbles** (with via-email / delivery-status / stored-not-emailed indicators), fallback to the legacy history timeline for pre-R01 tickets, a **Sync Inbound Email** header button (toast summary), unread dots, and the **Resolution Summary editor stays reachable right after Resolved/Closed** (a pinned ticket keeps the thread pane + Resolution editor open after Close). **Public email branding cleanup (Phase 6)**: `lib/email/brevo.ts` `wrapHtml` header/footer no longer hardcode "License Management" — a configurable `BRANDING_TAGLINE` (default "Software Development & Client Support") is used; the internal API's own email templates (`app/internal/backend/admin/email/templates/seed/route.ts`) and license-specific copy (`license_revoked` sentence) are intentionally untouched. **Public email rendering cleanup (Phase 7)**: pure helpers in `lib/tickets/email.ts` — `renderCustomerMessageHtml` (Markdown table blocks → real HTML tables, all other text HTML-escaped so no raw `| :-: |`/`| - |` markup or scripts reach customers) and `renderCustomerMessagePlain` (strips table separator/alignment rows) — now used by `support_reply` / `sales_reply` (`brevo.ts`) and `resolutionHtmlBody`. Boundary respected: no `/internal/api/*`, no SMTP/IMAP engine, queue, schema, auth, notification or storefront changes. Verified: `npx tsc --noEmit` 0 errors, `npm run build` green (all routes present incl. `/api/tickets/inbound` + `/api/tickets/[id]/read`), render helpers unit-verified (tables converted, separators stripped, HTML escaped). NOT deployed; awaits user approval.) | 100% |
...
| **Admin Messages Query Inbox — R01 Phase 3: Incoming Email → Chat Final Fix (AWS-01 R01, 2026-08-16)** | ✅ Applied (completes client-email → Messenger Chat direction; NO `/internal/api/*`/licensing/auth/storefront changes). (1) Admin identity: missing/generic admin display names (`"Admin User"`, `"Websmith Team"`, `"Websmith Support Team"`, `"Websmith Support"`, `"Support Team"`, empty) normalized at RENDER TIME to **"Websmith Digital Support"** (`ADMIN_SENDER_LABEL` in `app/admin/messages/AdminMessagesClient.tsx`); real names preserved; outbound Brevo `from`/`name` untouched. (2) Body-only in chat: `POST /api/tickets/inbound` stores `message` = `parsed.text || parsed.html` only (never envelope/header/signature-auth metadata); existing Message-ID dedupe + chronological `messages[]` ordering keep the client reply appearing once. (3) Incoming attachments: `parsed.attachments` → attachment BYTES stored in the shared `uploads` collection (`storeInboundAttachments`, max 10MB, shared `validateAttachmentFiles` policy) and linked to the new `messages` row (`attachments:[{name,url,size,contentType}]`); the ORIGINAL email+attachments stay in the support mailbox untouched (IMAP READ-ONLY, never Seen/deletes) so it reaches `support@websmithdigital.com` directly, and the stored bytes guarantee chat can never lose an attachment; Messenger Chat renders a compact read-only indicator (name + size + `GET /api/uploads/<id>`); no composer/resend UI for inbound attachments. `ThreadMessage.attachments?` (4) **Auto-poll for live chat** (`AdminMessagesClient.tsx`): when a conversation is open, a 30-second silent interval polls the existing `/api/tickets/inbound` IMAP sync; if new client messages are matched (`result.matched > 0`), the open thread is refreshed via `refreshOpenTicket()` so the reply appears in Messenger Chat immediately — no manual Sync Inbound click required; the manual button remains for immediate sync. (5) **Reply Thread + Resolved Preview clear on send** (`AdminMessagesClient.tsx`): after `handleReply` succeeds, `setGreetingKey("")` is called alongside `setReply("")` so the "Resolved Preview" card is removed (not left stale); selecting a new template re-generates the preview from ticket data, and the custom-edited textarea content is what gets sent — one reply = one outgoing email. added in `core/services/ticketService.ts`. Files: `app/api/tickets/inbound/route.ts`, `app/admin/messages/AdminMessagesClient.tsx`, `core/services/ticketService.ts`. `npx tsc --noEmit` EXIT 0; `npx next build` EXIT 0. |
| **Admin Messages Query Inbox - R01 Phase 4: Inbound Email -> Chat auto-sync (3s) + mailbox routing fix (AWS-01 R01, 2026-08-17)** | ✅ Applied (the client email reply now appears in Messenger Chat automatically within ~3 seconds and the reply->support@ routing dependency is closed in code; NO `/internal/api/*`/licensing/auth/storefront changes). (1) **Sync Inbound button REMOVED** (`AdminMessagesClient.tsx`) - the Query Conversation header has no manual button anymore; only the silent background auto-poll remains (no toast, no visible loader). (2) **Auto-poll every 3 s**: `POLL_INTERVAL_MS = 3_000`, first poll ~800 ms after open; polls ONLY while a conversation is selected AND `status` is not closed; stops on deselect/unmount; an in-flight ref guard (`pollInFlight`) never lets two IMAP sweeps overlap; transport switched to `quietFetch` (`syncInboundEmail` in `core/services/ticketService.ts`) so a session expiry can never kill the page and the poll is fully silent - refresh (`refreshOpenTicket()`) happens only when `matched > 0`. (3) **Chat bubble shows ONLY client name + body + timestamp + attachment link** for inbound client email - `senderEmail` and the "via email" tag are no longer rendered on client bubbles (admin delivery indicators unchanged); bodies are cleaned at STORE time by `cleanInboundBody()` in `app/api/tickets/inbound/route.ts` (quoted `>` blocks, "On ... wrote:", `-----Original Message-----`, Outlook `From:/Sent:/To:` header blocks, `--` signatures, "Sent from my iPhone/...") plus a display-only client mirror (`cleanClientBody`) for pre-fix stored rows; duplicate protection (Message-ID dedupe) and chronological ordering unchanged. (4) **Inbound sync hardening** (`inbound/route.ts`): each poll processes only the NEWEST `MAX_UNSEEN_BATCH` (40) UNSEEN messages so the 3 s poll stays light with a backlog (mail is opened READ-ONLY, never marked Seen; dedupe makes re-processing harmless); a module-level in-flight guard skips overlapping syncs (`skipped:true`); the sender fallback now honors thread identity - among the sender's tickets the one whose normalized subject matches the inbound subject (Re:/Fwd: stripped) wins, newest-updated tiebreak (NEVER subject alone; sender match is still the gate). (5) **Reply -> support@ routing dependency fixed in code** (`app/api/tickets/[id]/replies/route.ts`): the admin reply's From is resolved by `resolveReplySender()` to an **enabled PostgreSQL `mailboxes` row whose address equals the resolved support address** (Manage Page `contact_info.email` -> env fallback -> `support@websmithdigital.com`) and passed as the existing `sendEmail` `from` override - so the client's reply loops back into the SAME inbox the inbound sync polls (Client -> support@/configured inbox -> inbound parser -> ticket -> Messenger Chat). When no enabled mailbox matches, the default support sender is used unchanged. Honest mailbox dependency: the inbound parser reads ONLY enabled PG `mailboxes` (IMAP READ-ONLY, UNSEEN); if support@ mail does not land in such a mailbox (MX/forwarding or credentials), the route reports it and the chat shows no inbound message - no fake mailbox state anywhere. Files: `app/api/tickets/inbound/route.ts`, `app/api/tickets/[id]/replies/route.ts`, `core/services/ticketService.ts`, `app/admin/messages/AdminMessagesClient.tsx`. Verified: `npx tsc --noEmit` EXIT 0, `npx next build` EXIT 0, cleaner 8/8 unit samples. NOT deployed; awaits user approval + live mailbox verification (real external Gmail/Outlook reply). | 100% |
| **Admin Messages Query Inbox - R01 Phase 5: FINAL FAST INBOUND CHAT - 1-second poll (AWS-01 R01, 2026-08-17)** | ✅ Applied (the client email reply now lands in Messenger Chat automatically within ≤1 s and never later than the 3-second maximum; the 3-second polling interval was deliberately NOT used; NO `/internal/api/*`/licensing/auth/storefront/Brevo changes). The auto-poll interval is now **every 1 second** (`POLL_INTERVAL_MS = 1_000` in `app/admin/messages/AdminMessagesClient.tsx`, first poll ~800 ms after open); everything else from Phase 4 is preserved unchanged: no manual Sync Inbound button, no page refresh, no admin action; the silent background poll runs only while a conversation is selected AND `status` is not closed; client `pollInFlight` ref guard + server `syncInflight` guard never let two IMAP sweeps overlap; `quietFetch` transport so a session expiry can never kill the page; refresh (`refreshOpenTicket()`) only when `matched > 0`; body-only clean client bubbles (name + body + timestamp + `GET /api/uploads/<id>` attachment link); Message-ID dedupe; chronological `messages[]` order; existing ticket/client identity; outgoing Chat→client email (`[id]/replies` From resolves via `resolveReplySender()` to an ENABLED PostgreSQL `mailboxes` row whose address equals the resolved support address - `contact_info.email` → env fallback → `support@websmithdigital.com` - so the client's reply loops back into the same inbox the poll reads); templates, client ID, temporary credentials, existing ticket/message system and UI/UX all preserved. Production dependency (code-verified): the inbound sync reads ONLY enabled PG `mailboxes` (`is_enabled = TRUE`), so support@websmithdigital.com must be an enabled inbound mailbox; Brevo config untouched. Files: `app/admin/messages/AdminMessagesClient.tsx` (interval + comments), `core/services/ticketService.ts` + `app/api/tickets/inbound/route.ts` (comment sync only). Verified: `npx tsc --noEmit` EXIT 0, `npx next build` EXIT 0. Deployed 2026-08-17. |
| **Admin Messages Query Inbox — R01 FINAL FIX: Brevo Inbound Webhook for native support@ (AWS-01 R01, 2026-08-17)** | ✅ Applied (closes the root cause: support@websmithdigital.com is a NATIVE system account — IMAP = n/a, Sync = n/a — so the IMAP poller on /api/tickets/inbound can NEVER see client replies to support@; the app had NO inbound receiver for native mail at all. Verified facts: websmithdigital.com MX = mx1/mx2.privateemail.com (Namecheap Private Email — support@ is a real inbox there, mail already arrives); Brevo is used ONLY for sending (api.brevo.com/v3/smtp/email); Brevo inbound parsing webhook (type: inbound, events: [inboundEmailProcessed], top-level items[], PascalCase fields: MessageId/InReplyTo/From{Address,Name}/To[]/Recipients[]/SentAtDate/Subject/RawTextBody/RawHtmlBody/Attachments[{Name,ContentType,ContentLength,DownloadToken}]/Headers) requires a receiving DOMAIN different from the sending domain with MX → inbound1/inbound2.sendinblue.com. — so support@ mail is bridged via a dedicated receiving subdomain + forwarding, NEVER by touching the websmithdigital.com MX. **New receiver** POST /api/brevo/inbound (app/api/brevo/inbound/route.ts, public, token-gated: x-inbound-token header or ?token= must equal the BREVO_INBOUND_WEBHOOK_TOKEN env var — env missing → 503 BREVO_INBOUND_WEBHOOK_TOKEN is not configured so a missing config is loud; mismatch → 401): parses the Brevo items[] payload, keeps ONLY items addressed to the support account (isSupportRecipient: exactly the resolved support address contact_info.email → MAIL_SUPPORT_ADDRESS env → support@websmithdigital.com OR support@*.websmithdigital.com — sales@/no-reply@ never matched), skips senders that ARE support@ (bounce/loop guard) and items not addressed to support, downloads each attachment via Brevo GET /api.brevo.com/v3/inbound/attachments/{downloadToken} (api-key, 15 s timeout, 10 MB cap, best-effort — the original mail keeps its attachments at support@), then feeds EVERY item through the SAME shared pipeline lib/tickets/inbound-core.ts (processInboundEmail: In-Reply-To/References → messages.providerMessageId thread match, sender-verified contactEmail fallback, Message-ID dedupe, cleanInboundBody body-only cleaning, storeInboundAttachments → shared uploads collection, messages[] push + client_reply history + lastClientReplyAt + closed → in_progress reopen). The IMAP route app/api/tickets/inbound/route.ts was refactored to import the SAME core module (zero behavior change) — one pipeline, two transports. **UI** (app/admin/messages/AdminMessagesClient.tsx): refreshOpenTicket() is now DIFF-based (compares updatedAt/lastClientReplyAt/messages.length — state touched only when the ticket actually changed) and the 1-second auto-poll ALWAYS re-checks the open thread after the IMAP sync — so webhook-delivered mail (which carries no matched-count signal) appears in Messenger Chat within ≤1 s, max 3 s, silently. Outbound unchanged: admin replies stay support@websmithdigital.com only (sales@/no-reply@ never used); templates/client ID/temp credentials/resolution/unsubscribe/status/identity all preserved. Files: NEW app/api/brevo/inbound/route.ts, NEW lib/tickets/inbound-core.ts, app/api/tickets/inbound/route.ts (refactor-only), app/admin/messages/AdminMessagesClient.tsx. Verified: npx tsc --noEmit EXIT 0, npx next build EXIT 0 (294 pages). **External config REQUIRED before live mail flows** (see the "Brevo Inbound Webhook — required external configuration" note below): (1) env BREVO_INBOUND_WEBHOOK_TOKEN; (2) Brevo dashboard/API: inbound webhook type:"inbound" events:["inboundEmailProcessed"] url:"https://www.websmithdigital.com/api/brevo/inbound" domain:"reply.websmithdigital.com" + header x-inbound-token: <token>; (3) DNS: reply.websmithdigital.com MX 10 inbound1.sendinblue.com. + MX 20 inbound2.sendinblue.com. (websmithdigital.com MX untouched — support@ keeps receiving at Namecheap); (4) support@ (Namecheap Private Email) forwarding rule → support@reply.websmithdigital.com (forwarded copy; the ORIGINAL stays in the support@ inbox). NOT deployed; awaits user approval + the external configuration above. |
| **Admin Messages Query Inbox — R01 FINAL FIX: REAL inbound transport (read-only IMAP for native support@) + FAST Query Inbox (AWS-01 R01, 2026-08-17)** | ✅ Applied (the Brevo bridge is NOT operational — no MX for `reply.websmithdigital.com`, no `BREVO_INBOUND_WEBHOOK_TOKEN` env → deployed webhook answers 503 — and provider research proved the ONLY real receiving mechanism Namecheap Private Email (Open-Xchange) offers is standard IMAP/POP3 (`mail.privateemail.com:993` SSL; NO inbound webhook, NO message-reading API — Namecheap API = DNS/mailbox CRUD only). **REAL TRANSPORT**: `POST /api/tickets/inbound` (`app/api/tickets/inbound/route.ts`) now ALSO polls the native support@ mailbox READ-ONLY with provider credentials from env vars (`nativeSupportMailbox()`: `MAIL_SUPPORT_IMAP_HOST` default `mail.privateemail.com`, `MAIL_SUPPORT_IMAP_PORT` default 993, `MAIL_SUPPORT_IMAP_SECURE` default true — `"false"` for STARTTLS/143, `MAIL_SUPPORT_IMAP_USERNAME` default `MAIL_SUPPORT_ADDRESS` → `support@websmithdigital.com`, `MAIL_SUPPORT_IMAP_PASSWORD` REQUIRED — the SINGLE provider-side dependency: the support@ mailbox password from the Namecheap Private Email dashboard; without it the native poll is skipped and the summary's `errors[]` reports the exact missing config, never silent). The native mailbox flows through the SAME `syncMailbox()` + `processInboundEmail()` pipeline (ticket matching, `cleanInboundBody`, attachment storage in `uploads`, Message-ID dedupe, `messages[]`), IMAP opened READ-ONLY (never marks Seen, never mutates the mailbox → webmail + Internal Communications Center unaffected). Constraints honored: support@/sales@/no-reply@ unchanged, websmithdigital.com MX untouched, NO new mailbox/row, NO schema change, NO IMAP wiring into the Internal API mailbox system (native accounts stay IMAP = n/a there); the Brevo webhook route stays untouched but is no longer required. **FAST QUERY INBOX** (`app/admin/messages/AdminMessagesClient.tsx` + `core/services/ticketService.ts`): (1) the 1-second auto-poll's `refreshOpenTicket()` now uses `getTicketsQuiet()` (new quietFetch-transport list fetch, same response shape as `getTicketsPaged`) so a session expiry can NEVER page-replace the admin mid-poll; (2) every ⋮ action (Open / Close / Delete / Resend) runs independently with a per-action local loading state (`busyAction`/`busyTicketId` — spinner only on the in-flight menu item) and updates the affected card IN PLACE from the full-ticket API response (`applyTicketUpdate`; Delete removes only that card + decrements `total`) — Query Inbox never reloaded, no scroll reset, no layout/width change, conversation pane untouched; (3) Send Reply applies the returned updated ticket in place instead of a full list refresh. Ticket selection was already instant (list object, no detail fetch — account card unchanged). Files: `app/api/tickets/inbound/route.ts`, `lib/tickets/inbound-core.ts` (header comment only), `core/services/ticketService.ts`, `app/admin/messages/AdminMessagesClient.tsx`. Verified: `npx tsc --noEmit` EXIT 0, `npx next build` EXIT 0. NOT deployed; awaits user approval + setting `MAIL_SUPPORT_IMAP_PASSWORD` (provider-side dependency) and a real reply test (Client Gmail/Outlook → support@ → read-only IMAP poll → ticket message in Messenger Chat within ≤1 s). |
| **Admin Messages Query Inbox — R01 LIVE: env configured, transport verified, deployed (AWS-01 R01, 2026-08-17)** | ✅ Applied + DEPLOYED (the receive pipeline is LIVE end-to-end). (1) **Production env configured** — `MAIL_SUPPORT_IMAP_HOST=mail.privateemail.com`, `MAIL_SUPPORT_IMAP_PORT=993`, `MAIL_SUPPORT_IMAP_SECURE=true`, `MAIL_SUPPORT_IMAP_USERNAME=support@websmithdigital.com`, `MAIL_SUPPORT_IMAP_PASSWORD=<support@ mailbox password>` added to Vercel production (Sensitive, via CLI) + `.env.local` (never committed). (2) **Transport verified with REAL credentials** — read-only login to `mail.privateemail.com:993` (Dovecot IMAP4rev1, authorized TLS cert, `AUTH=LOGIN` supported) PASSED; INBOX opened READ-ONLY; the 2 UNSEEN messages parsed via `simpleParser` (UID 23 = self-sent "General Inquiry" from support@, UID 24 = Namecheap "Meet your new inbox" welcome mail — neither is a client reply); mailbox never mutated. (3) **Pipeline traced + verified against production data** — `nativeSupportMailbox()` (`app/api/tickets/inbound/route.ts`) + shared `processInboundEmail()` (`lib/tickets/inbound-core.ts`) are the exact code path the deployed Query Inbox polls every 1 s; real production ticket `6a82a9f8303d8f7cb4a340c2` ("AI Voice Agent", keemodatabox@gmail.com, OPEN) confirmed with the stored admin-reply `providerMessageId <202608170628.52498552064@smtp-relay.mailin.fr>` — an In-Reply-To reply from keemodatabox@gmail.com matches by thread, sender verified, body cleaned, attachment stored, timestamp kept, Message-ID dedupe. (4) **Deployed 2026-08-17** — `npx tsc --noEmit` EXIT 0, `npm run build` EXIT 0, `vercel --prod` green (env vars bound to the new deployment); production routes verified live: `POST /api/tickets/inbound` + `GET /api/tickets` answer 401 (auth gate) — no 500s. (5) **Transient Atlas incident observed (NOT code)** — during verification `ac-o1abmjb-shard-00-02.wr0yzts.mongodb.net` (the cluster PRIMARY) intermittently rejected TLS (`tlsv1 alert internal error`, alert 80) from BOTH Vercel (`MongoServerSelectionError` 500s on `/api/tickets`, `/api/tickets/inbound`, `/api/users/notifications`) and the dev machine (raw TLS + driver); shards 00-00/00-01 stayed healthy; status.mongodb.com showed NO incident; the failure self-resolved within the hour (all 3 shards TLS OK, full-URI driver connect PASS, `writablePrimary: true` on shard-00-02) and production returned to 401-gaited health — no code/env change was made for this. (6) **Final acceptance STILL REQUIRED** — real end-to-end: reply from a real Gmail/Outlook client (e.g. reply to the admin email on ticket "AI Voice Agent" / keemodatabox@gmail.com, with an attachment) → support@websmithdigital.com → read-only IMAP poll → same Query Ticket → Messenger Chat shows body + attachment + timestamp, no duplicate, within ≤1 s (max 3 s). Outgoing Chat → support@ → client unchanged (`[id]/replies` From = enabled mailbox matching the resolved support address, else default support sender). | 90% (awaits real-mail acceptance) |
| **Admin Messages Query Inbox — R01 ROUTING DATA FIX: contact_info swap (AWS-01 R01, 2026-08-17)** | ✅ Applied + VERIFIED LIVE (DATA-ONLY fix — no code, no UI, no templates, no Brevo, no MX, no mailbox changes). ROOT CAUSE: the production `settings` document `contact_info` had `email` = **sales@websmithdigital.com** and `sales_email` = **support@websmithdigital.com** (inverted). Because the universal Brevo sender logic derives `support_reply`/`admin_notification`/`conversation_created` From from `contactInfo.email` (`getSupportAddress`, lib/email/brevo.ts:41-49 + 988-989) and `new_sales_enquiry`/`sales_reply` from `contactInfo.sales_email`, ALL Query Inbox admin replies were sent FROM sales@ — so client replies (Gmail "Reply" → the From address) landed in the **sales@ mailbox, which the Query Inbox inbound poll never reads** (it reads ONLY native support@ via `MAIL_SUPPORT_IMAP_*` + enabled PG `mailboxes` rows; built-in native accounts are never mailbox rows). VERIFIED chain (2026-08-17): Brevo accepted both admin replies (`deliveryStatus: "sent"`, messageIds `<202608170628.52498552064@smtp-relay.mailin.fr>` / `<202608170721.40186629963@smtp-relay.mailin.fr>`); support@ INBOX + Spam had ZERO messages since Aug 17 (nothing arrives because nothing is sent FROM support@); Namecheap SMTP probing (25/465/587) blocked (no banner → no mailbox-existence check possible). FIX: `$set {"value.email": "support@websmithdigital.com", "value.sales_email": "sales@websmithdigital.com"}` on the production `contact_info` record (matched 1, modified 1) — applied via a direct-connection probe of the writable primary (`directConnection=true` + `tlsAllowInvalidCertificates` on the one-shot script only) after a RECURRING Atlas TLS outage (`tlsv1 alert internal error` alert 80 on `ac-o1abmjb-shard-00-02`, the PRIMARY — secondaries healthy; ~16 min continuous rejection, same self-resolving pattern as earlier today; status.mongodb.com showed NO incident; production `/api/tickets` was 500 during the window, recovered to 401 after). VERIFIED LIVE: `GET https://www.websmithdigital.com/api/settings/public/contact_info` → `email: support@websmithdigital.com`, `sales_email: sales@websmithdigital.com` (HTTP 200, 2.4 s); `GET /api/tickets` → 401 (2.2 s). Universal semantics now hold everywhere (Brevo `getSupportAddress`/`getSalesAddress` are shared by public website + Query Inbox + Internal API Communications): **support@ = General Support / Query Inbox / TWO-WAY**; **sales@ = Sales / TWO-WAY**; **no-reply@ = OTP/system ONE-WAY** (`no_reply_email` empty in contact_info → falls back to `SENDER_EMAIL`/`MAIL_FROM_ADDRESS` as before — unchanged). Outstanding: real end-to-end client reply acceptance (send Gmail/Outlook reply → support@ → read-only IMAP poll → same ticket → Messenger Chat ≤1 s, body + attachment + timestamp, no duplicate). | 100% (data), 90% (acceptance) |
