...
| **R01 — Global Collaboration & Technical Excellence section — exact 810px columns + image/video dimensions (public homepage, UI-only, 2026-08-19)** | ✅ Applied (UI-only, `app/page.tsx` "Global Diversity & Collaboration" section). (1) **Columns widened to exactly 810px each** — the desktop grid `min-[1700px]:grid-cols-[780px_40px_780px]` → `[810px_40px_810px]` so both columns are side-by-side at 810px (40px gap preserved, not stacked, surrounding section design unchanged). (2) **Image → 810 × 550px** — the left column's `h-[680px]` → `h-[550px]` (width = 810px column, `object-cover`); heading + existing description + **Enterprise Grade / Diverse Talent** badges stay ABOVE the image (order unchanged, never moved below). (3) **Video → 810 × 600px** — the right column's `h-[680px]` → `h-[600px]` (width = 810px column, `object-cover`); "Why Websmith?" text stays ABOVE the video. (4) **Video behavior preserved**: already `autoPlay` + `loop` (plays on website load) + the existing pointerdown/keydown play-workaround effect (line ~687), **unmuted** (no `muted` attr — the sound/player requirement stays in place) and **native `controls` retained**. No text changed, no surrounding section changes, no backend/data/API changes. Files: `app/page.tsx` only. Verified: change is CSS dimension/class-only; `npx tsc --noEmit`/`next build` NOT runnable locally (node_modules/TypeScript not installed in this checkout). NOT deployed; awaits user approval. | 100% |
