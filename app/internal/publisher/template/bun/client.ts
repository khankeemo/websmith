...
export class ApiClient {
  ...
  constructor(config: Record<string, any>, hardware?: HardwareDetector, cache?: CacheManager) {
    ...
    this.timeout = Math.floor(parseInt(this.apiConfig.timeout || '30000', 10));
  ...
  private async _signRequest(payload: Record<string, any>, method: string, path: string, query: string): Promise<Record<string, string>> {
    ...
    const signature = await signRequest(payload, this.apiSecret, timestamp, nonce, method, path, query);
    return { 'x-api-key': this.apiKey, 'x-timestamp': timestamp, 'x-nonce': nonce, 'x-signature': signature };
  }
  ...
  async validateLicense(licenseKey: string, hardwareId?: string): Promise<Record<string, any>> {
    ...
    if (this._cache && response.success && response.data?.valid) {
      await this._cache.setLicenseStatus(response);
    }
    return response;
  }
  ...
  async activateLicense(licenseKey: string, hardwareId?: string): Promise<Record<string, any>> {
    if (!hardwareId) hardwareId = this._getHardwareId();
    const response = await this._request('license', { action: 'activate', license_key: licenseKey, hardware_id: hardwareId });
  ...
  async replaceDevice(licenseKey: string, newHardwareId?: string, oldHardwareId?: string): Promise<Record<string, any>> {
    ...
    const response = await this._request('device', { action: 'replace', license_key: licenseKey, old_hardware_id: oldHardwareId, new_hardware_id: newHardwareId });
    if (this._cache) await this._cache.invalidateLicenseStatus();
    return response;
  }
