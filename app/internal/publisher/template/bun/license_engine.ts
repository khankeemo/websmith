...
export class LicenseEngine {
  ...
  async activate(licenseKey: string): Promise<Record<string, any>> {
    ...
    if (result.success) { this._licenseKey = licenseKey; await this.initialize(); }
    return result;
  }
  ...
  async convertTrial(plan?: string, customerName?: string, customerEmail?: string): Promise<Record<string, any>> {
    ...
    if (result.success) { if (result.license_key) this._licenseKey = result.license_key; await this.initialize(); }
    return result;
  }
  ...
  async replaceHardware(): Promise<Record<string, any>> {
    ...
    if (result.success) { await this._cache.invalidateLicenseStatus(); this._status = null; await this.initialize(); }
    return result;
  }
