...
export function getCppTemplates(context: PublisherContext): Record<string, string> {
  ...
  return {
    'client.hpp': `#ifndef WEBSMITH_CLIENT_HPP
...
class CacheManager {
public:
    explicit CacheManager(const std::string& productId = "${productId}")
...
public:
    LicenseEngine(const std::string& apiKey, const std::string& apiSecret = "",
        ...
        , m_cache(std::make_unique<CacheManager>(
              config.value("product_id", "${productId}"))) {
...
project(${cmakeProject}-sdk VERSION ${kitVersion})
find_package(CURL REQUIRED)
find_package(nlohmann_json REQUIRED)
find_package(OpenSSL REQUIRED)
add_library(websmith_sdk INTERFACE)
target_include_directories(websmith_sdk INTERFACE include)
target_link_libraries(websmith_sdk INTERFACE CURL::libcurl nlohmann_json::nlohmann_json OpenSSL::SSL OpenSSL::Crypto)
`,
    'README.md': `# ${productName} — C++ SDK

## Overview

The C++ SDK provides header-only integration with the ${productName} licensing API. Uses \`curl\`, \`nlohmann/json\`, and OpenSSL for HMAC-SHA256 signing.
