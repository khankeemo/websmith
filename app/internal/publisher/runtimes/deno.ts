...
export function getDenoTemplates(context: PublisherContext): Record<string, string> {
  ...
  return {
    'client.ts': `/**
 * ${context.productName} SDK — Deno License Management Client
 * Generated by Websmith License API Center
 * Runtime: deno | Version: ${context.kitVersion}
 */
...
class Client {
  ...
  async replaceDevice(
    ...
    if (newHardwareId === undefined) newHardwareId = await this._hardware.getFingerprint();
    if (oldHardwareId === undefined) {
      throw new ApiError(400, 'old_hardware_id is required for device replacement');
    }
    const payload: Record<string, unknown> = {
      action: 'replace',
      license_key: licenseKey,
    ...
    const response = await this.request('device', payload);
    if (this._cache) this._cache.invalidateLicenseStatus();
    return response;
  }
}
...
class LicenseEngine {
  ...
  async replaceHardware(): Promise<Record<string, unknown>> {
    if (!this._licenseKey) {
      throw new Error('License key unavailable. Please activate first.');
    }
    const newHardwareId = await this._hardware.getFingerprint();
    let oldHardwareId = this._status?.hardwareId || null;
    if (!oldHardwareId) {
      const cached = this._cache.getLicenseStatus<Record<string, unknown>>();
      if (cached && cached.hardware_id) {
        oldHardwareId = cached.hardware_id as string;
      }
    }
    if (!oldHardwareId) {
      throw new Error('Current hardware_id unavailable. Cannot replace device.');
    }
    if (oldHardwareId === newHardwareId) {
      return { success: false, message: 'Old and new hardware IDs are identical.' };
    }
    const result = await this._client.replaceDevice(
      this._licenseKey,
      newHardwareId,
      oldHardwareId,
    );
...
`,
  ...
  "exports": "./client.ts",
  "tasks": {
    "check": "deno check client.ts",
    "fmt": "deno fmt client.ts"
  },
  "imports": {}
}`,
    'README.md': `# ${context.productName} SDK (Deno)

## Version
${context.kitVersion}

## Overview
Production-ready Deno SDK for ${context.productName} license management.
...
### Replace Hardware
\`\`\`typescript
const result = await engine.replaceHardware();
\`\`\`
