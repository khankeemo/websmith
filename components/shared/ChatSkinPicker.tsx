...
.ws-skin-trigger{width:32px;height:32px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--sk-input-border);background:var(--sk-input-bg);color:var(--sk-accent);cursor:pointer;transition:border-color 160ms ease,box-shadow 160ms ease;}
.ws-skin-trigger:hover,.ws-skin-trigger[aria-expanded="true"]{border-color:var(--sk-focus-border);box-shadow:var(--sk-focus-ring);}
.ws-skin-panel{position:absolute;top:calc(100% + 8px);right:0;z-index:60;width:min(300px,calc(100vw - 48px));max-height:min(420px,58dvh);display:flex;flex-direction:column;background:var(--sk-picker-bg);border:1px solid var(--sk-picker-border);border-radius:14px;box-shadow:var(--sk-picker-shadow);overflow:hidden;animation:wsSkinPanelIn .16s ease;transform-origin:top right;}
.ws-skin-panel-head{flex-shrink:0;display:flex;align-items:center;justify-content:space-between;padding:10px 12px 8px;border-bottom:1px solid var(--sk-picker-border);}
.ws-skin-panel-title{font-size:11px;font-weight:800;letter-spacing:0.6px;text-transform:uppercase;color:var(--sk-subtitle);}
.ws-skin-panel-count{font-size:10px;color:var(--sk-time);}
...
.ws-skin-row:hover{background:var(--sk-system-bg);}
.ws-skin-row-active{border-color:var(--sk-accent);}
.ws-skin-meta{min-width:0;display:flex;flex-direction:column;gap:1px;}
.ws-skin-name{display:inline-flex;align-items:center;gap:5px;font-size:12px;font-weight:700;color:var(--sk-text);}
.ws-skin-desc{font-size:10.5px;line-height:1.4;color:var(--sk-subtitle);}
.ws-skin-swatch{position:relative;width:44px;height:34px;flex-shrink:0;border-radius:8px;overflow:hidden;border:1px solid var(--sk-picker-border);}
                ...
                >
                  ...
                  <span className="ws-skin-meta">
                    <span className="ws-skin-name">
                      {skin.name}
                      {active && <CheckCircle2 size={12} color="var(--sk-accent)" aria-hidden="true" />}
                    </span>
